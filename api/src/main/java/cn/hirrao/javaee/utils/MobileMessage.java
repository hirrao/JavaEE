...
public class MobileMessage {
    ...
    public static Client createClient() throws Exception {
        var config = new Config().setAccessKeyId(System.getenv("ALIBABA_CLOUD_ACCESS_KEY_ID")).setAccessKeySecret(System.getenv("ALIBABA_CLOUD_ACCESS_KEY_SECRET"));
    ...
    public static int sendMessage(String phoneNumber, String code, String time) {
        try {
            var client = MobileMessage.createClient();
            var sendSmsRequest = new com.aliyun.dysmsapi20170525.models.SendSmsRequest().setSignName(System.getenv("SIGN_NAME")).setTemplateCode(System.getenv("TEMPLATE_CODE")).setPhoneNumbers(phoneNumber).setTemplateParam(String.format("{\"code\":\"%s\",\"time\":\"%s\"}", code, time));
