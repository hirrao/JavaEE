...
public interface BloodPressureMapper extends BaseMapper<BloodPressure> {
    @Select("SELECT DATE(recordTime) as recordTime, AVG(SBP) as SBP , AVG(DBP) as DBP FROM bloodPressureLog WHERE uid= #{uid} AND DATE(recordTime) BETWEEN DATE_SUB(#{date}, INTERVAL 6 DAY)  AND #{date} GROUP BY DATE(recordTime)")
